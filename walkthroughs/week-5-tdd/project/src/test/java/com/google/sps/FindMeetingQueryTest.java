...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void everyAttendeeIsConsideredIgnoresOptional() {
    // Have each person have different events. We should see three options because each person has
    // split the restricted times. Ignores the optional attendee because optional attendee cannot
    // attend during times where mandatory attendees are both free.
    // Events  : |---------optional C----------|
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void everyAttendeeIsConsideredIncludesOptional() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times. Includes optional attendee because there are two options that
    // the mandatory and optional attendees can all attend.
    // Events  :             |opt C|
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|                 |--3--|
  ...
  @Test
  public void justEnoughRoomIgnoresOptional() {
    // Have one mandatory person, but make it so that there is just enough room at one point in the day to
    // have the meeting. Ignore optional attendee because the schedule would result in a time slot smaller
  ...
  public void onlyOptionalAttendees() {
      // No mandatory attendees, just two optional attendees with several gaps in their schedule.
      // Should see 2 options.
  ...
  @Test
  public void onlyOneOptionalCanAttend() {
      // Mandatory attendees and two optional attendees but only one optional attendee can attend,
      // so return times that all mandatory and the one optional can attend.
  ...
  @Test
  public void maximizesAttendance() {
      // Not all optional attendees can attend at the same time, one list of results will
      // allow one of the optional attendees to attend, the other will allow two. Return
      // the timeranges that two optional attendees can attend.
      ...
      Collection<Event> events = Arrays.asList(
          new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_1000AM, false),
            Arrays.asList(PERSON_A)),
          new Event("Event 2", TimeRange.fromStartEnd(TIME_1030AM, TimeRange.END_OF_DAY, true),
  ...
  @Test
  public void maximizesAttendanceFindsBestSolutionFirst() {
      // Not all optional attendees can attend at the same time, one list of results will
      // allow one of the optional attendees to attend, the other will allow two. Return
      // the timeranges that two optional attendees can attend.
      // Will find optimal solution at beginning before trying other solutions

      // Events  : |--opt A--|        
      // Events  : |--opt B--|---C----|--opt D--|
      // Day     : |----------------------------|
      // Options : |---------|
